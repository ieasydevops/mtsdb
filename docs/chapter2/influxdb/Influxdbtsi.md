...
为了支持大规模的时间序列 -即单时间序列高基数的数据存储，Influx 增加了时间序列索引。通过TSI的存储引擎，用户能够处理上百万的独立的时间序列。
...
Heapster项目和类似的用例没有解决的问题是限制了SHOW查询返回的数据的范围。我们将在将来更新查询语言，以便按时间限制这些结果。我们也不能解决让所有这些系列的读写都很热的问题。对于这个问题，扩展集群是解决方案。我们需要在查询语言中添加护栏和限制，并最终将增加，溢出到磁盘的处理逻辑。这项工作将在infloxdb的每次发行的版本中更新。
